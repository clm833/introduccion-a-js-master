...
$nuestroTitulo.innerText = "Hola, cambiamos el título con JS";
...
for (i=0; i<$mediaLinks.length; i++ ) {
    console.log(`$mediaLinks[${i}] = ${$mediaLinks[i].innerText}`);
}
...
console.log(`textContent: ${$nuestroTitulo.textContent}`)
console.log(`innerText: ${$nuestroTitulo.innerText}`)
...
$nuestroTitulo.style.color = 'gray';
...
const $cabeceraPagina = document.querySelector('header')
const $nuevoLogo = document.createElement('img')
$nuevoLogo.src = 'img/woman_bw.jpg';
$cabeceraPagina.appendChild($nuevoLogo);
